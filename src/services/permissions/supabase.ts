...
export async function loadCurrentPermissions(supabase: SupabaseClient, moduleKey: string): Promise<{ permissions: PermissionSet; role: string; error: string | null }> {
  ...
  if (workspace.role === "super_admin" || workspace.role === "workspace_admin" || workspace.role === "admin") return { error: null, permissions: allowAll, role: workspace.role };
...
export function defaultsForRole(role: string): PermissionSet {
  if (role === "super_admin" || role === "workspace_admin" || role === "admin") return allowAll;
