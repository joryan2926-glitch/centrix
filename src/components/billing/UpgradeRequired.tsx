...
const priceIds: Record<PlanCode, string> = {
...
export function UpgradeRequired({ moduleKey, requiredPlan }: { moduleKey: string; requiredPlan: PlanCode }) {
  ...
  async function upgrade() {
    ...
    try {
      ...
      setError(checkoutError instanceof Error ? checkoutError.message : "Mise a niveau impossible.");
      setLoading(false);
    }
  }
  ...
  return (
    <main className="grid min-h-[calc(100vh-9rem)] place-items-center">
      <Card className="max-w-xl p-8 text-center">
        <div className="mx-auto grid h-16 w-16 place-items-center rounded-[20px] bg-blue-50 text-blue-600"><ShieldCheck size={30} /></div>
